//This is just the code for blacklisting the links, discord, client, token and the rest ARENT defined.

//Feel free to edit the link bodies and add new ones to your code as well. Im open for accepting new feature request, just DM me.
...
//remove the above lines as well as this one
...
client.on('message', message => {
  ...
  const grabberEmbed = new Discord.MessageEmbed()
	grabberEmbed.setTitle("**IP Grabber Detected!**")
	grabberEmbed.addField("**Username :** ", `${message.author.username}`)
	grabberEmbed.addField("**User ID :** ", `${message.author.id}`)
	grabberEmbed.addField("**Domain :** Grabify", `**Link sent :** ||${message}||`)
	grabberEmbed.setTimestamp()
  
  
  if ((message.content.includes("https://dlscord.org/nitro-gift")) || (message.content.includes("http://dlscord.org/nitro-gift"))) {
    message.delete();
    message.channel.send(grabberEmbed)
  }
...
if ((message.content.includes("https://headshot.monster")) || (message.content.includes("http://headshot.monster"))) {
    message.delete();
    message.channel.send(grabbedEmbed);
  }
